...
// Aksi CRUD hanya untuk Admin
if ($_SESSION['level'] === 'Admin') {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if ($_POST['aksi'] === 'tambah') {
            $stmt = $conn->prepare("INSERT INTO pegawai (nama, nik, tanggal_lahir, jenis_kelamin, alamat, telepon, email, jabatan_id, cabang_id, status, tanggal_masuk)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
...
// Data jabatan dan cabang (untuk form)
    ...
    <table id="pegawaiTable" class="table table-striped table-bordered">
        ...
        <tbody>
            <?php foreach ($pegawais as $row): ?>
                <?php if ($_SESSION['level'] === 'Pegawai' && $_SESSION['username'] !== $row['email']) continue; ?>
                <tr>
...
<!-- CDN scripts assumed already included in header.php -->
<script>
