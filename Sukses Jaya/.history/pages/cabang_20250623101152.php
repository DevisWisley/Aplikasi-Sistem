...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_POST['aksi'] === 'tambah') {
        ...
        $stmt->execute([
            $_POST['nama_cabang'], $_POST['alamat'], $_POST['kota'],
            $_POST['telepon'], $_POST['keterangan'], $_POST['email']
        ]);
    }
    ...
    if ($_POST['aksi'] === 'edit') {
        ...
        $stmt->execute([
            $_POST['nama_cabang'], $_POST['alamat'], $_POST['kota'],
            $_POST['telepon'], $_POST['keterangan'], $_POST['email'], $_POST['cabang_id']
        ]);
    }
}
...
<div class="p-4 w-100">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?= $cabang['email'] ?></td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='editCabang(<?= json_encode($cabang) ?>)'><i class="fas fa-edit"></i></button>
          <button class="btn btn-danger btn-sm" onclick='hapusCabang(<?= $cabang["cabang_id"] ?>)'><i class="fas fa-trash"></i></button>
          <button class="btn btn-info btn-sm" onclick='showDetail(<?= json_encode($cabang) ?>)'><i class="fas fa-eye"></i></button>
        </td>
      </tr>
      <?php endforeach; ?>
    </tbody>
