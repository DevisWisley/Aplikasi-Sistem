...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_SESSION['level'] === 'Admin') {
...
<div class="p-4 w-100">
    ...
    <table id="pegawaiTable" class="table table-striped table-bordered">
        <thead class="table-dark">
            <tr>
                <th>Nama</th>
                <?php if ($_SESSION['level'] === 'Admin'): ?><th>NIK</th><?php endif; ?>
                <th>Jabatan</th>
                <th>Cabang</th>
                <?php if ($_SESSION['level'] === 'Admin'): ?>
                    <th>Status</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <?php endif; ?>
                <td>
                    <button class="btn btn-info btn-sm" onclick='showDetail(<?= json_encode($row) ?>)'><i class="fas fa-eye"></i></button>
                    <?php if ($_SESSION['level'] === 'Admin'): ?>
                    <button class="btn btn-warning btn-sm" onclick='editPegawai(<?= json_encode($row) ?>)'><i class="fas fa-edit"></i></button>
                    <button class="btn btn-danger btn-sm" onclick='hapusPegawai(<?= $row['pegawai_id'] ?>)'><i class="fas fa-trash"></i></button>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; ?>
...
<script>
  ...
  function hapusPegawai(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = 'pegawai.php?delete=' + id;
      }
    });
  }
  ...
  function editPegawai(data) {
    ...
    document.getElementById('edit_id').value = data.pegawai_id;
    for (let key in data) {
  ...
  function showDetail(data) {
    ...
    for (let key in data) {
      content += `<li class="list-group-item"><strong>${key}:</strong> ${data[key]}</li>`;
    }
    content += '</ul>';
